...
const syncCards = async (forceUpdate = false) => {
  ...
  try {
    ...
    const insert = db.prepare(
      `INSERT ${conflictClause} INTO cards (id, name, domain, type, supertype, tags, rarity, energy, plain_text, rich_text, set_id, label, power, might, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    );
      ...
      allCards.forEach((card) => {
        if (card && card.id) {
          ...
          if (Array.isArray(card.classification?.domain)) {
            domainStr = card.classification.domain
              .map((d) =>
                typeof d === "object"
                  ? d.name || d.label || JSON.stringify(d)
                  : d,
              )
              .join(", ");
            ...
            domainStr =
              typeof card.classification.domain === "object"
                ? card.classification.domain.name ||
                  ...
                  JSON.stringify(card.classification.domain)
